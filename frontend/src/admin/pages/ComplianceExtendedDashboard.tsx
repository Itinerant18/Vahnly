...
function badge(status: string) {
  const cls = STATUS_BADGE[status] ?? 'bg-canvas-soft text-body';
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-mute">Document verification, expiry tracking, privacy requests, and regulatory compliance</p>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <div className="space-y-3">
        ...
        <div className="flex gap-1 flex-wrap">
          {['ALL', 'PENDING', 'UNDER_REVIEW', 'APPROVED', 'REJECTED'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs border ${statusFilter === s ? 'bg-accent text-white border-accent' : 'bg-canvas border-canvas-soft text-body hover:text-ink'}`}>
              {s}
            </button>
          ))}
        </div>
        <div className="bg-canvas rounded-xl border border-canvas-soft overflow-hidden">
          ...
          {!loading && drivers.length === 0 && <div className="p-6 text-center text-sm text-mute">Queue is empty.</div>}
          <div className="divide-y divide-canvas-soft/50">
            {drivers.map(d => (
              <button key={d.id} onClick={() => openDetail(d.id)}
                className={`w-full text-left px-4 py-3 hover:bg-canvas-soft/30 ${selected?.id === d.id ? 'bg-accent/5 border-l-2 border-accent' : ''}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-ink">{d.name}</div>
                    <div className="text-xs text-mute">{d.phone} · {d.city_prefix}</div>
                  </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-mute">{selected.phone} · DL: {selected.dl_number}</div>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {['Driving License', 'ID Proof', 'Address Proof', 'Selfie'].map(doc => {
                  ...
                  return (
                    <div key={doc} className="bg-canvas-soft/50 rounded-lg p-3 text-center">
                      <div className="text-2xl mb-1">{url ? '📄' : '❌'}</div>
                      <div className="text-xs text-body">{doc}</div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-sm text-body">Show documents expiring within</span>
        {[30, 60, 90].map(d => (
          <button key={d} onClick={() => setDays(d)}
            className={`px-3 py-1.5 rounded-lg text-sm border ${days === d ? 'bg-accent text-white border-accent' : 'bg-canvas border-canvas-soft text-body hover:text-ink'}`}>
            {d} days
          </button>
        ))}
        <span className="text-xs text-mute ml-auto">{docs.length} document{docs.length !== 1 ? 's' : ''}</span>
      </div>

      {loading && <div className="text-sm text-mute animate-pulse">Loading…</div>}

      <div className="space-y-2">
        {docs.length === 0 && !loading && (
          <div className="bg-canvas rounded-xl border border-canvas-soft p-8 text-center text-mute text-sm">
        ...
        {docs.map(doc => {
          ...
          return (
            <div key={doc.id} className={`rounded-xl border p-4 flex items-center justify-between gap-4 ${urgency}`}>
              <div>
                <div className="text-sm font-medium text-ink">{doc.display_name}</div>
                <div className="text-xs text-mute">{doc.entity_type} · {doc.entity_id.slice(0, 8)}… · {doc.doc_type}</div>
                <div className="flex gap-1 mt-1 flex-wrap">
                  {doc.tags.map(tag => <span key={tag} className="text-[10px] border border-canvas-soft rounded px-1.5 py-0.5 text-mute">{tag}</span>)}
                </div>
              </div>
    ...
    <div className="space-y-4">
      ...
      {showNew && (
        <div className="bg-canvas rounded-xl border border-canvas-soft p-4 space-y-3">
          <div className="text-sm font-semibold text-ink">New Privacy Request</div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-mute">Request Type</label>
            ...
            <div>
              <label className="text-xs text-mute">Requester Type</label>
            ...
            <div>
              <label className="text-xs text-mute">Phone</label>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowNew(false)} className="px-4 py-1.5 border border-canvas-soft rounded-lg text-sm text-body hover:bg-canvas-soft">Cancel</button>
          </div>
        </div>
      )}
      ...
      {processingId && (
        <div className="bg-canvas rounded-xl border border-canvas-soft p-4 space-y-3">
          <div className="text-sm font-semibold text-ink">Process Request</div>
          <div className="flex gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setProcessingId(null)} className="px-4 py-1.5 border border-canvas-soft rounded-lg text-sm text-body">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        {requests.map(req => {
          ...
          return (
            <div key={req.id} className="bg-canvas rounded-xl border border-canvas-soft p-4 flex items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-ink">{req.request_type}</span>
                  {badge(req.status)}
                </div>
                <div className="text-xs text-mute mt-0.5">{req.requester_type} · {req.requester_email}</div>
                {req.notes && <div className="text-xs text-body mt-1 italic">"{req.notes}"</div>}
                {req.processed_by_email && <div className="text-xs text-mute">Processed by {req.processed_by_email}</div>}
              </div>
        ...
        {requests.length === 0 && !loading && (
          <div className="text-sm text-mute text-center py-8">No privacy requests matching this filter.</div>
        )}
      </div>
    </div>
  );
  ...
  <div className="space-y-4">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {[
        ...
        <div key={item.title} className="bg-canvas rounded-xl border border-canvas-soft p-5">
          <div className="text-2xl mb-2">{item.icon}</div>
          <div className="font-semibold text-sm text-ink">{item.title}</div>
          <div className="text-xs text-mute mt-1">{item.desc}</div>
