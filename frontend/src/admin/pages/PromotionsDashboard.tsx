...
export const PromotionsDashboard: React.FC = () => {
	...
	return (
		<div className="w-full h-full overflow-y-auto p-6 space-y-6">
			<div>
				<h1 className="text-2xl font-bold tracking-tight text-ink">Promotions & Loyalty Management</h1>
				<p className="text-xs text-mute mt-1">Manage corporate marketing promo codes, bulk CSV registrations, active app banners, referral parameters, and loyalty brackets</p>
			</div>
			...
			<div className="flex border-b border-canvas-soft bg-canvas rounded-xl p-1 shadow-sm max-w-lg">
				<button
					...
					className={`flex-1 h-9 rounded-pill text-xs font-semibold transition-colors ${
						activeTab === 'promos' ? 'bg-canvas-soft text-ink border border-canvas-soft' : 'text-mute hover:text-ink'
					}`}
				>
					Promo Codes
				</button>
				<button
					...
					className={`flex-1 h-9 rounded-pill text-xs font-semibold transition-colors ${
						activeTab === 'banners' ? 'bg-canvas-soft text-ink border border-canvas-soft' : 'text-mute hover:text-ink'
					}`}
				>
					App Banners
				</button>
				<button
					...
					className={`flex-1 h-9 rounded-pill text-xs font-semibold transition-colors ${
						activeTab === 'referrals' ? 'bg-canvas-soft text-ink border border-canvas-soft' : 'text-mute hover:text-ink'
					}`}
				>
					Referral Engine
				</button>
				<button
					...
					className={`flex-1 h-9 rounded-pill text-xs font-semibold transition-colors ${
						activeTab === 'loyalty' ? 'bg-canvas-soft text-ink border border-canvas-soft' : 'text-mute hover:text-ink'
					}`}
				>
					Loyalty Tiers
				</button>
			...
			{activeTab === 'promos' && (
				<div className="space-y-6 animate-fade-in">
					...
					<div className="flex flex-wrap items-center justify-between gap-4 bg-canvas p-4 rounded-xl border border-canvas-soft shadow-sm">
						<div className="flex items-center space-x-4">
							<div className="flex flex-col">
								<label className="text-[10px] uppercase font-bold text-mute mb-1 font-sans">Filter Status</label>
								<select
							...
							<div className="flex flex-col">
								<label className="text-[10px] uppercase font-bold text-mute mb-1 font-sans">Filter Shard</label>
								<select
									className="h-8 rounded-pill bg-canvas-soft border border-canvas-soft px-3 text-xs text-ink focus:outline-none focus:border-ink font-mono font-semibold"
									value={cityFilter}
							...
							<div className="flex flex-col">
								<label className="text-[10px] uppercase font-bold text-mute mb-1 font-sans">User Segment</label>
								<select
									className="h-8 rounded-pill bg-canvas-soft border border-canvas-soft px-3 text-xs text-ink focus:outline-none focus:border-ink font-mono font-semibold"
									value={segmentFilter}
						...
						<div className="flex space-x-2">
							<button
								...
								className="border border-canvas-soft hover:border-ink text-xs font-semibold rounded-pill h-8 px-4 transition-colors"
							>
								Bulk Upload CSV
							</button>
							<button
					...
					<div className="bg-canvas rounded-xl border border-canvas-soft overflow-hidden shadow-sm">
						{promosLoading ? (
							<div className="p-12 text-center text-xs text-mute animate-pulse">Fetching promotions database registry...</div>
						) : filteredPromos.length === 0 ? (
							<div className="p-12 text-center">
								<div className="text-sm font-semibold text-ink">No promotions registered matching filters</div>
								<p className="text-xs text-mute mt-1">Configure a new code or adjust filters to list registry keys.</p>
							</div>
						) : (
							<table className="w-full text-left border-collapse">
								<thead>
									<tr className="border-b border-canvas-soft bg-canvas-soft">
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Code</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Type</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Value</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Caps (Max / Min)</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Segment</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute text-center">Redemptions</th>
										<th className="p-4 text-[10px] font-semibold uppercase tracking-wider text-mute">Validity Range</th>
								...
								<tbody className="divide-y divide-canvas-soft text-xs">
									{filteredPromos.map((p) => (
										<tr key={p.code} className="hover:bg-canvas-softer transition-colors">
											<td className="p-4 font-mono font-bold text-ink text-sm tracking-wide">{p.code}</td>
											<td className="p-4 font-semibold text-body">{p.promo_type}</td>
											<td className="p-4 font-mono text-ink font-bold">
												{p.promo_type === 'PERCENT' ? `${p.value}%` : `₹${p.value}`}
											</td>
											<td className="p-4 font-mono text-body space-y-0.5">
												<div>Max: {formatPaise(p.max_discount_paise)}</div>
												<div className="text-[10px] text-mute">Min Fare: {formatPaise(p.min_fare_paise)}</div>
											</td>
											<td className="p-4">
												<span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold bg-canvas-soft text-ink">
													{p.user_segment}
												</span>
											</td>
											...
											<td className="p-4 font-mono text-body space-y-0.5 text-[11px]">
											...
											<td className="p-4 text-right space-x-1 space-y-1">
												<button
													...
													className="border border-canvas-soft hover:border-ink text-[10px] font-bold px-2 py-1 rounded"
												>
												...
												{p.status === 'ACTIVE' ? (
													<button
														...
														className="bg-canvas-soft text-status-warn border border-canvas-soft hover:border-status-warn text-[10px] font-bold px-2 py-1 rounded"
													>
														Pause
													</button>
													...
													<button
														...
														className="bg-ink text-on-dark hover:bg-black-elevated text-[10px] font-bold px-2 py-1 rounded"
													>
														Resume
													</button>
												) : null}
												{p.status !== 'EXPIRED' && (
													<button
														...
														className="bg-canvas-soft text-status-alert border border-canvas-soft hover:border-status-alert text-[10px] font-bold px-2 py-1 rounded"
													>
														Expire
													</button>
												)}
			...
			{activeTab === 'banners' && (
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
					...
					<div className="lg:col-span-2 bg-canvas p-6 rounded-xl border border-canvas-soft shadow-sm space-y-6">
						<div>
							<h2 className="text-sm font-bold text-ink uppercase tracking-wider">Active Promotional Banners</h2>
							<p className="text-[11px] text-mute mt-0.5">Manage messages broadcasted to passengers inside regional app shards</p>
						</div>

						{bannersLoading ? (
							<div className="text-xs text-mute animate-pulse">Loading configurations...</div>
						) : (
							<div className="space-y-4">
								{banners.length === 0 ? (
									<div className="p-8 text-center text-xs text-mute border border-dashed border-canvas-soft rounded-lg">
										No banners currently configured. Create a banner campaign on the right.
									</div>
								) : (
									<div className="space-y-4">
										{banners.map((b) => (
											<div key={b.id} className="p-4 rounded-lg bg-canvas-soft border border-canvas-soft space-y-3 flex flex-col justify-between">
												<div className="flex justify-between items-start">
													<div>
														<span className="font-mono text-[10px] font-bold bg-ink text-on-dark px-1.5 py-0.5 rounded mr-2 uppercase">
															{b.city_prefix}
														</span>
													...
													<div className="flex space-x-2">
														<button
															...
															className={`text-[10px] font-bold px-2 py-0.5 rounded ${
																b.is_active ? 'bg-status-online/15 text-status-online border border-status-online/20' : 'bg-canvas text-mute border border-canvas-soft'
															}`}
														>
												...
												<p className="text-xs text-ink font-semibold italic bg-canvas p-2.5 rounded border border-canvas-soft">
													"{b.banner_text}"
												</p>
											</div>
										))}
					...
					<div className="bg-canvas p-5 rounded-xl border border-canvas-soft shadow-sm space-y-4 h-fit">
						<div>
							<h3 className="text-xs font-bold text-ink uppercase tracking-wider">New Banner Campaign</h3>
							<p className="text-[10px] text-mute mt-0.5">Publish an auto-applied promotion banner</p>
						</div>

						<div className="space-y-3 text-xs">
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">City Shard Code</label>
								<select
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Banner Message Text</label>
								<textarea
									placeholder="e.g. Surge discount applied for BLR today!"
									className="w-full min-h-20 rounded bg-canvas-soft border border-canvas-soft p-2.5 text-xs text-ink placeholder:text-mute focus:outline-none focus:border-ink font-semibold"
							...
							<div className="grid grid-cols-2 gap-2">
								<div>
									<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Start Range</label>
									<input
										type="datetime-local"
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-[10px] text-ink font-mono"
										value={newBanner.valid_from}
										onChange={(e) => setNewBanner({ ...newBanner, valid_from: e.target.value })}
									/>
								</div>
								<div>
									<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">End Range</label>
									<input
										type="datetime-local"
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-[10px] text-ink font-mono"
			...
			{activeTab === 'referrals' && (
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
					...
					<div className="lg:col-span-2 bg-canvas p-6 rounded-xl border border-canvas-soft shadow-sm space-y-6">
						<div>
							<h2 className="text-sm font-bold text-ink uppercase tracking-wider">Referral Reward & Fraud Rules</h2>
							<p className="text-[11px] text-mute mt-0.5">Control reward payouts triggered by customer invitations and set validation safeguards</p>
						</div>

						{referralsLoading || !referralSettings ? (
							<div className="text-xs text-mute animate-pulse">Loading referral settings...</div>
						) : (
							<div className="space-y-6">
								{/* Fraud Controls */}
								<div className="p-4 rounded-lg bg-canvas-soft border border-canvas-soft space-y-3">
									<h3 className="text-xs font-bold text-ink uppercase tracking-wider">Fraud Prevention Filters</h3>
									<div className="space-y-2">
										<label className="flex items-center space-x-3 text-xs text-body font-semibold cursor-pointer">
											<input
												type="checkbox"
												className="rounded bg-canvas border-canvas-soft text-ink focus:ring-0"
								...
								<div className="space-y-3">
									<h3 className="text-xs font-bold text-ink uppercase tracking-wider">Reward Distribution Rules</h3>
									{referralSettings.rules.length === 0 ? (
										<div className="p-8 text-center text-xs text-mute border border-dashed border-canvas-soft rounded-lg">
											No reward distribution rules registered. Add one on the right.
										</div>
									) : (
										<div className="bg-canvas border border-canvas-soft rounded-lg overflow-hidden">
											<table className="w-full text-left border-collapse text-xs">
												<thead>
													<tr className="border-b border-canvas-soft bg-canvas-soft">
														<th className="p-3 text-[10px] font-semibold uppercase text-mute">Participant Flow</th>
														<th className="p-3 text-[10px] font-semibold uppercase text-mute">Trigger Event</th>
														<th className="p-3 text-[10px] font-semibold uppercase text-mute">Reward Type</th>
														<th className="p-3 text-[10px] font-semibold uppercase text-mute">Reward Value</th>
												...
												<tbody className="divide-y divide-canvas-soft">
													{referralSettings.rules.map((rule, idx) => (
														<tr key={idx} className="hover:bg-canvas-softer transition-colors">
															...
															<td className="p-3 font-semibold text-body">
																{rule.reward_type}
															</td>
															...
															<td className="p-3 text-right">
																<button
																	...
																	className="text-status-alert font-semibold hover:underline"
																>
																	Remove
																</button>
															</td>
					...
					<div className="bg-canvas p-5 rounded-xl border border-canvas-soft shadow-sm space-y-4 h-fit">
						<div>
							<h3 className="text-xs font-bold text-ink uppercase tracking-wider">Create Reward Rule</h3>
							<p className="text-[10px] text-mute mt-0.5">Register a trigger mapping and reward release</p>
						</div>

						<div className="space-y-3 text-xs">
							<div className="grid grid-cols-2 gap-2">
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Inviter Role</label>
									<select
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-ink font-semibold"
								...
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Invited Role</label>
									<select
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-ink font-semibold"
							...
							<div className="grid grid-cols-2 gap-2">
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Trigger Event</label>
									<select
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-ink font-semibold"
								...
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Trigger Event Count</label>
									<input
										type="number"
							...
							<div className="grid grid-cols-2 gap-2">
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Reward Medium</label>
									<select
										className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 text-ink font-semibold"
								...
								<div>
									<label className="block text-[9px] uppercase text-mute mb-1 font-semibold">Reward Value (₹)</label>
									<input
										type="number"
										step="1"
			...
			{/* TAB: LOYALTY TIERS */}
			{activeTab === 'loyalty' && (
				<div className="bg-canvas p-6 rounded-xl border border-canvas-soft shadow-sm space-y-6 animate-fade-in">
					<div>
						<h2 className="text-sm font-bold text-ink uppercase tracking-wider">Loyalty Tier Thresholds & Perks</h2>
						<p className="text-[11px] text-mute mt-0.5">Define completed trip goals for passenger tiers and assign app perks matrices</p>
					</div>

					{loyaltyLoading || !loyaltySettings ? (
						<div className="text-xs text-mute animate-pulse">Loading loyalty metrics configuration...</div>
					) : (
						<div className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
								{loyaltySettings.tiers.map((tier, idx) => (
									<div key={tier.tier_name} className="p-5 bg-canvas-soft border border-canvas-soft rounded-xl space-y-4">
										<div className="flex justify-between items-center pb-2 border-b border-canvas-soft">
											<span className="font-mono text-sm font-black text-ink tracking-wide">{tier.tier_name}</span>
											<span className="text-[10px] text-mute font-bold uppercase">Tier Matrix</span>
										</div>

										<div className="space-y-3 text-xs">
											<div>
												<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Min Completed Trips</label>
												<input
													type="number"
													className="w-full h-8 rounded bg-canvas border border-canvas-soft px-2.5 font-mono text-ink font-bold"
											...
											<div>
												<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Perk Discount Rate (%)</label>
												<input
													type="number"
													step="0.5"
													className="w-full h-8 rounded bg-canvas border border-canvas-soft px-2.5 font-mono text-ink font-bold"
											...
											<div className="space-y-2 pt-2 border-t border-canvas-soft">
												<label className="flex items-center space-x-3 text-body font-semibold cursor-pointer">
													<input
														type="checkbox"
														className="rounded bg-canvas border-canvas-soft text-ink focus:ring-0"
			...
			{showCreateModal && (
				<div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
					<div className="bg-canvas border border-canvas-soft rounded-xl max-w-xl w-full p-6 space-y-6 shadow-2xl relative">
						<button
							onClick={() => setShowCreateModal(false)}
							className="absolute top-4 right-4 text-mute hover:text-ink text-lg font-bold"
						>
							&times;
						</button>

						<div>
							<h3 className="text-sm font-bold text-ink uppercase tracking-wider">Configure Promotional Code</h3>
							<p className="text-[11px] text-mute mt-0.5">Commit a new promo coupon key parameters</p>
						</div>

						<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Promo Code (Uppercase)</label>
								<input
									type="text"
									placeholder="e.g. MONSOON30"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono font-bold text-ink focus:outline-none focus:border-ink"
									value={newPromo.code}
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Category Type</label>
								<select
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 font-semibold text-ink focus:outline-none"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Discount Value (rupees/percent)</label>
								<input
									type="number"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono font-bold text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Max Discount Allowed (₹)</label>
								<input
									type="number"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono font-bold text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Min Ride Fare Requirement (₹)</label>
								<input
									type="number"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">User Segment Shard</label>
								<select
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 font-semibold text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Global Usage Cap Limit</label>
								<input
									type="number"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Usage Cap Limit Per User</label>
								<input
									type="number"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2.5 font-mono text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Activation Start Date</label>
								<input
									type="datetime-local"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 font-mono text-ink"
							...
							<div>
								<label className="block text-[9px] uppercase tracking-wider text-mute mb-1 font-semibold">Expiration End Date</label>
								<input
									type="datetime-local"
									className="w-full h-8 rounded bg-canvas-soft border border-canvas-soft px-2 font-mono text-ink"
						...
						<div className="flex items-center space-x-3 text-xs pt-2">
							<label className="flex items-center space-x-2 text-body font-semibold cursor-pointer">
								<input
									type="checkbox"
									className="rounded bg-canvas border-canvas-soft text-ink focus:ring-0"
						...
						<div className="flex justify-end space-x-2 border-t border-canvas-soft pt-4">
							<button
								onClick={() => setShowCreateModal(false)}
								className="border border-canvas-soft hover:border-ink text-xs font-semibold rounded-pill h-9 px-4 transition-colors"
							>
								Cancel
							</button>
							<button
			...
			{showBulkModal && (
				<div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
					<div className="bg-canvas border border-canvas-soft rounded-xl max-w-xl w-full p-6 space-y-6 shadow-2xl relative">
						...
						<div>
							<h3 className="text-sm font-bold text-ink uppercase tracking-wider">Bulk CSV Importer</h3>
							<p className="text-[11px] text-mute mt-0.5">Paste comma-separated rows mapping code definitions directly to Redis</p>
						</div>

						<div className="space-y-2">
							<label className="block text-[10px] uppercase tracking-wider text-mute font-semibold">CSV Text Input</label>
							<textarea
								placeholder="code,promotype,value,minfare&#10;DISCOUNT50,PERCENT,50,5000&#10;FLAT100,FLAT,100,8000"
								className="w-full min-h-60 rounded bg-canvas-soft border border-canvas-soft p-3 font-mono text-xs text-ink placeholder:text-mute focus:outline-none focus:border-ink font-semibold"
								value={bulkCsvText}
								onChange={(e) => setBulkCsvText(e.target.value)}
							/>
							<div className="text-[10px] text-mute">
								Headers required: <code className="font-mono text-ink font-bold">code,promotype,value,minfare</code>. Value and minfare are rupee-denominated floats (e.g. 5000 paise = 50 rupees, but enter raw 50 rupees or 5000 paise depending on handler fallback, the backend defaults to paise integers).
							</div>
						</div>
						...
						<div className="flex justify-end space-x-2 border-t border-canvas-soft pt-4">
							...
							<button
								...
								className="bg-ink text-on-dark text-xs font-semibold rounded-pill h-9 px-5 hover:bg-black-elevated transition-colors disabled:opacity-50"
								disabled={bulkUploading}
							>
			...
			{selectedPromoAnalytics && (
				<div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
					<div className="bg-canvas border border-canvas-soft rounded-xl max-w-md w-full p-6 space-y-6 shadow-2xl relative">
						<button
						...
						<div>
							<h3 className="text-sm font-bold text-ink uppercase tracking-wider">Performance Analytics: {selectedPromoAnalytics.code}</h3>
							<p className="text-[11px] text-mute mt-0.5">Dynamic marketing KPIs computed using standard SHA/FNV hashes</p>
						</div>

						<div className="grid grid-cols-1 gap-4 divide-y divide-canvas-soft">
							<div className="pt-2 flex justify-between items-center text-xs">
								<span className="text-mute font-semibold">Total Redemptions</span>
								<span className="font-mono font-bold text-ink text-base">{selectedPromoAnalytics.redemptions}</span>
							</div>

							<div className="pt-4 flex justify-between items-center text-xs">
								<span className="text-mute font-semibold">Gross Value GMV Impact</span>
								<span className="font-mono font-bold text-ink text-base">{formatPaise(selectedPromoAnalytics.gmv_impact_paise)}</span>
							</div>

							<div className="pt-4 flex justify-between items-center text-xs">
								<span className="text-mute font-semibold">Marketing Campaign ROI</span>
						...
						<div className="bg-canvas-soft p-3 rounded-lg border border-canvas-soft text-[10px] text-mute font-mono">
							Redemption counts and marketing ROI calculations are linked directly to active Redis hash metrics and transaction audits.
						</div>

						<div className="flex justify-end border-t border-canvas-soft pt-4">
							<button
								onClick={() => setSelectedPromoAnalytics(null)}
								className="bg-ink text-on-dark text-xs font-semibold rounded-pill h-9 px-6 hover:bg-black-elevated transition-colors"
							>
