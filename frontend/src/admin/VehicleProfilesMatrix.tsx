...
export const VehicleProfilesMatrix: React.FC = () => {
  ...
  return (
    <div className="bg-background-tertiary rounded-xl p-6 border border-background-secondary shadow-sm space-y-6 lg:col-span-3">
      <div>
        <h2 className="text-lg font-bold text-content-primary font-move">Car Owner Asset & Profile Management</h2>
        <p className="text-xs text-content-secondary">Govern transmission validation controls, escrow ledgers, and vehicle tier classifications</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-3 max-h-[400px] overflow-y-auto pr-2">
          <div className="text-[10px] uppercase tracking-wider font-bold text-content-tertiary">Verified Vehicle Owners ({profiles.length})</div>
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${
                  selectedProfile?.id === profile.id ? 'bg-white/20 text-white' : 'bg-background-secondary text-content-primary'
                }`}>
                  {profile.transmission_requirement}
                </span>
              </div>
              <div className={`text-[11px] mt-1 font-medium ${selectedProfile?.id === profile.id ? 'text-white/80' : 'text-content-primary'}`}>
                {profile.vehicle_make_model}
              </div>
              <div className={`text-[9px] font-mono mt-1 ${selectedProfile?.id === profile.id ? 'text-white/60' : 'text-content-tertiary'}`}>
        ...
        <div className="lg:col-span-2 bg-white border border-background-secondary rounded-xl p-6 flex flex-col justify-between min-h-[400px]">
          {selectedProfile ? (
            <div className="space-y-6 flex-grow flex flex-col justify-between text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="border-b border-background-secondary pb-3">
                    <div className="text-[10px] uppercase font-bold text-content-tertiary tracking-wider">Demand Profile context</div>
                    <div className="text-base font-bold text-content-primary font-move mt-1">{selectedProfile.owner_name}</div>
                    <div className="text-xs text-content-secondary font-mono mt-0.5">{selectedProfile.owner_phone}</div>
                  </div>
                  <div className="space-y-2 text-xs">
                    <div><span className="font-bold text-content-secondary">Vehicle Asset:</span> <span className="text-content-primary font-medium">{selectedProfile.vehicle_make_model}</span></div>
                    <div><span className="font-bold text-content-secondary">License Registry:</span> <span className="font-mono text-content-primary bg-background-tertiary px-1.5 py-0.5 rounded border border-background-secondary/60">{selectedProfile.license_plate}</span></div>
                    
                    {/* Embedded Escrow Tracking Sub-Panel Matrix */}
                    <div className="pt-3 border-t border-background-secondary/60 mt-3">
                      <span className="block text-[10px] uppercase font-bold text-content-tertiary tracking-wider mb-1">Prepaid Account Escrow Ledger</span>
                ...
                <div className="space-y-5">
                  <div>
                    <div className="text-[10px] uppercase font-bold text-content-tertiary tracking-wider mb-2">Transmission Failsafe Constraint</div>
                  ...
                  <div>
                    <div className="text-[10px] uppercase font-bold text-content-tertiary tracking-wider mb-2">Marketplace Asset Pricing Tier</div>
              ...
              <div className="border-t border-background-secondary pt-4 mt-4">
                <div className="text-[10px] uppercase font-bold text-content-tertiary tracking-wider mb-3">Vehicle Documentation & Insurance Verification Status</div>
                <div className="flex gap-2">
              ...
              <div className="border-t border-background-secondary pt-4 mt-6">
                <button
                  onClick={commitAssetProfileChanges}
                  disabled={isUpdating}
                  type="button"
                  className="w-full bg-black hover:bg-gray-800 text-white font-bold py-3.5 px-4 rounded-full transition text-xs uppercase tracking-wider disabled:opacity-40 cursor-pointer active:scale-95"
                >
