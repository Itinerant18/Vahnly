      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden">
          <div className="space-y-1">
            <span className="text-zinc-500 text-[9px] uppercase font-mono tracking-wider font-bold">{t('walletBalance')}</span>
            <h3 className="text-3xl font-mono font-bold text-emerald-400">₹{balance.toFixed(2)}</h3>
            <span className="text-[8px] font-mono text-zinc-650 block pt-0.5">{t('autoDebitActive')}</span>
          </div>
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-5 space-y-4 font-mono text-xs text-white">
          ...
          <div className="space-y-2">
            <label className="block text-[8px] text-zinc-500 uppercase font-mono">{t('quickPresetAmounts')}</label>
          ...
          <form onSubmit={handleAddMoney} className="space-y-2 pt-2 border-t border-zinc-900">
            <label className="block text-[8px] font-bold text-zinc-500 uppercase font-mono tracking-wider">{t('customAmount')}</label>
            <div className="flex gap-2">
              <input
                type="number"
                value={addVal}
