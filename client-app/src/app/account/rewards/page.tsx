    ...
    <div className="space-y-6 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <form onSubmit={handleApplyPromo} className="bg-zinc-950 border border-zinc-900 rounded-2xl p-5 space-y-4 font-mono text-xs">
          <h4 className="text-xs font-bold text-white uppercase border-b border-zinc-900 pb-2">{t('applyPromoCode')}</h4>

          <div className="space-y-2">
            <label className="block text-[8px] text-zinc-500 uppercase">{t('couponVoucherCode')}</label>
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-5 flex items-center justify-between gap-6 font-mono text-xs text-zinc-400">
          <div className="space-y-3 flex-grow">
            <span className="text-zinc-600 block text-[8px] uppercase font-bold border-b border-zinc-900 pb-1.5">{t('loyaltyRewardsTier')}</span>
          ...
          {promoList.map((p) => (
            <div key={p.code} className="py-3 flex justify-between items-center text-xs font-mono">
              <div>
                ...
                <span className="text-zinc-500 text-[8px] block mt-0.5">{t('codeExpiry', { code: p.code, expiry: p.expiry })}</span>
              </div>
