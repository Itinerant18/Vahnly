  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-zinc-900">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-white font-move">Saved Places</h2>
          <p className="text-zinc-500 text-[10px] font-mono uppercase tracking-wider mt-0.5">Manage favorite addresses for one-click route booking inputs</p>
        </div>
        ...
        >
          {showAddForm ? 'Close Form' : 'Add Place'}
        </button>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={handleAddPlace} className="bg-zinc-950 border border-zinc-900 rounded-2xl p-5 space-y-4 animate-fadeIn font-mono text-xs">
          <h4 className="text-xs font-bold text-white uppercase border-b border-zinc-900 pb-2">Add Favorite Location</h4>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-[8px] text-zinc-500 uppercase mb-1">Label Tag</label>
              <select
                value={newLabel}
              ...
              >
                ...
                <option>🛒 Mall</option>
                <option>✈️ Airport</option>
              </select>
            </div>
            <div>
              <label className="block text-[8px] text-zinc-500 uppercase mb-1">Address Details</label>
              <input
                type="text"
                value={newAddress}
