...
func (s *ContractionHierarchiesService) ComputeShortestPathETA(ctx context.Context, sourceID, targetID int64) (float64, error) {
	...
	// Optimization: Removed sync.Mutex that previously wrapped priority queue pops/pushes
	// since all search state variables here are function-local and not accessed concurrently.
	// This avoids unnecessary lock contention overhead in the tight algorithmic loop.
