...
func (s *ContractionHierarchiesService) ComputeShortestPathETA(ctx context.Context, sourceID, targetID int64) (float64, error) {
	...
	// Bolt Optimization: Removed unnecessary sync.Mutex (mu). The Dijkstra search
	// state (priority queues and distance maps) is local to this function invocation,
	// so no parallel threads access these structures. Removing locks eliminates massive
	// overhead inside the algorithmic tight loop.
